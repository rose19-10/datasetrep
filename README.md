...
TO ACCESS API FROM INTERNET. FOLLOW THESE STEPS

API endpoint:  https://data.cdc.gov/resource/rsk5-566a.json
...
class CardiovascularController extends Controller
{
    ...
    {
        $data = $this->cdcService->filterByState($state);
        return response()->json($data);
    }

    public function byYear($year)
    {
        $data = $this->cdcService->filterByYear($year);
        return response()->json($data);
    }
}
...
# Test API directly to browser

https://data.cdc.gov/resource/rsk5-566a.json?$limit=10